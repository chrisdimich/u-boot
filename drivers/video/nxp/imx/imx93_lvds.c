...
/* 71700Khz should be replaced with your panel pixel clock */
#define serial_clk 71700*7000
...
static int imx93_lvds_phy_power_on(struct udevice *dev)
{
       ...
       unsigned int lvds_val, ldb_val = 0;
       bool bg_en;
       debug("%s\n", __func__);
...
static int imx93_ldb_probe(struct udevice *dev)
{
       ...
       if (dev->plat_ == NULL) {
               priv->base = (void *)0x4ac10000;//media blk base address
               ...
               if(priv->conn_dev && device_get_uclass_id(priv->conn_dev) == UCLASS_PANEL){
                       ...
                       // fake pwm-adp5585 driver, so we can't use real pwm
                     ...
                     const struct display_timing *timing)
{
       ...
       if (dev->plat_ == NULL) {
		...
		debug("%s: display_enable panel_bpp %i\n", __func__, panel_bpp);
		msleep(2000);
               display_enable(dev->parent, panel_bpp, &priv->timings);
		debug("%s: after display_enable\n", __func__);
               if (IS_ENABLED(CONFIG_VIDEO_BRIDGE)) {
                       if (priv->conn_dev &&
                               device_get_uclass_id(priv->conn_dev) == UCLASS_VIDEO_BRIDGE) {
